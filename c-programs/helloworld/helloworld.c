...
const char key[][8] = { "HWJTRHKM", "JNCTCTLW", "MSEQLIPV", "OCOTTHRS", "GSFPBQTK", "VVVXIBOU", "LSFWTUUJ", "OCEBJVUP", "IRWJBRMP", "GJOPCUDO" };
uint8_t result[16];
...
int main(void) {
	...
	*/
	...
	int c;
	c = a+b;
	//md5String(key[0], result);
	//clock_gettime(CLOCK_REALTIME, &end);
	//uint64_t nsecs1 = ((uint64_t)start.tv_sec) * 1000000000 + start.tv_nsec;
	//uint64_t nsecs2 = ((uint64_t)end.tv_sec)   * 1000000000 + end.tv_nsec;
	//uint64_t delta_nsecs = nsecs2 - nsecs1;
	//fprintf (stdout, "Simulation speed: %0" PRId64 " cycles, %0" PRId64 " nsecs", delta_cycles, d	  elta_nsecs);
	return 0;
}
